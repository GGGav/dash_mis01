...
resp = { provisioned: 0, active: 0, loggedIn: 0, outofdate: 0}

SCHEDULER.every '5s', :first_in => 0 do |job|
  respprev = resp
  resp = { provisioned: 40, active: 35 + (rand * 6).to_i, loggedIn: (rand * 3).to_i, outofdate: (rand * 3).to_i}
  ...
  if resp[:loggedIn] != respprev[:loggedIn]
    send_event('mis01_terminals_loggedIn', { value: resp[:loggedIn]} )
  end
